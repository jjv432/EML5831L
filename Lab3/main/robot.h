...
#define del_T      0.01 // [s]
#define wheelRad   0.06 // [m]
#define robotWidth 0.2762 // [m]
...
void fwdKinematics(void){
  ...
  // trig functions are very expensive
  robot_vel[0] = (joint_vel[0] + joint_vel[1])* wheelRad/2.0;
  robot_vel[1] =  (wheelRad/robotWidth)*(-joint_vel[0]+joint_vel[1]);
    
  robot_pos[0] = robot_pos[0] + robot_vel[0]*cos(robot_pos[2])*del_T;   // X position update
  robot_pos[1] = robot_pos[1] + robot_vel[0]*sin(robot_pos[2])*del_T;   // Y position update
  robot_pos[2] = robot_pos[2] + robot_vel[1]*del_T;                     // Theta position update

  robot_pos[2] = atan2(sin(robot_pos[2]),cos(robot_pos[2])); // [-pi,pi)
 
}

void invKinematics(void){
  
  des_joint_vel[0] = (1/wheelRad)*des_robot_vel[0] - (robotWidth/(2*wheelRad))*des_robot_vel[1];
  des_joint_vel[1] = (1/wheelRad)*des_robot_vel[0] + (robotWidth/(2*wheelRad))*des_robot_vel[1];
